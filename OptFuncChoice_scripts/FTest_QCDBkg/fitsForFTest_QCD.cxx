...
void fitsForFTest_QCD()
{
  ...
  //TString FuncType[2] = { "3rd_Poly","5th_Poly"};
  TString FuncType[2] = { "4th_Poly", "4th_exp"};
  //TString FuncType[1] = {"3rd_Poly" };
  double npars = 0;
  RooGenericPdf gp;
  for(int i=0; i<2; i++)
  ...
  //RooPlot* frame3 = lj1_mass.frame(Title("Unbinned data shown in default frame binning")) ;
    ...
    TString text_Chi2=Form("#chi^{2} = %0.3f",chi2*(100-npars));
    TString text_Chi2ndf=Form("#chi^{2}/ndf = %0.3f",chi2);
    TString text_Chi2prob=Form("#chi^{2} Probability (in %) = %0.3f",prob*100);
   ...
   TLatex *tex1 = new TLatex();
    if(FuncType[i]=="2nd_Poly")
    {
      tex1= new TLatex(lx,ly,"Dijet Function: #Sigma_{i=0 to 2} a_{i}*x^{i}");
    }
    else if(FuncType[i]=="3rd_Poly")
    {
      tex1= new TLatex(lx,ly,"Dijet Function: #Sigma_{i=0 to 3} a_{i}*x^{i}");
    }
    else if(FuncType[i]=="4th_Poly")
    {
      tex1= new TLatex(lx,ly,"Dijet Function: #Sigma_{i=0 to 4} a_{i}*x^{i}");
    }
    else if(FuncType[i]=="5th_Poly")
    {
      tex1= new TLatex(lx,ly,"Dijet Function: #Sigma_{i=0 to 5} a_{i}*x^{i}");
    }
    else if(FuncType[i]=="1st_exp")
    {
      tex1= new TLatex(lx,ly,"Dijet Function: exp(a_{1}*x/100)");
    }
    else if(FuncType[i]=="2nd_exp")
    {
      tex1= new TLatex(lx,ly,"Dijet Function: exp(#Sigma_{i=1 to 2} a_{i}*(x/100)^{i})");
    }
    else if(FuncType[i]=="3rd_exp")
    {
      tex1= new TLatex(lx,ly,"Dijet Function: exp(#Sigma_{i=1 to 3} a_{i}*(x/100)^{i})");
    }
    else if(FuncType[i]=="4th_exp")
    {
      tex1= new TLatex(lx,ly,"Dijet Function: exp(#Sigma_{i=1 to 4} a_{i}*(x/100)^{i})");
    }
    else if(FuncType[i]=="5th_exp")
    {
      tex1= new TLatex(lx,ly,"Dijet Function: exp(#Sigma_{i=1 to 5} a_{i}*(x/100)^{i})");
    }
    else if(FuncType[i]=="6th_exp")
    {
      tex1= new TLatex(lx,ly,"Dijet Function: exp(#Sigma_{i=1 to 6} a_{i}*(x/100)^{i})");
    }
    tex1->SetNDC();
  ...
  TLatex * AT = new TLatex(0.12,0.54,"#it{#bf{ATLAS}} Simulation");
  AT->SetNDC();
  AT->SetTextFont(42);
  AT->SetTextColor(1);
  AT->SetTextSize(0.04);
  ...
  TLatex * Sim = new TLatex(0.12,0.49," work-in-progress");
  Sim->SetNDC();
  Sim->SetTextFont(42);
  Sim->SetTextColor(1);
  ...
  TLatex * Tag = new TLatex(0.15,0.45,"Xbb 60% WP");
  Tag->SetNDC();
  Tag->SetTextFont(42);
  Tag->SetTextColor(1);
  ...
  probi->Draw();
   gPad->SaveAs("./FTestFits/"+FuncType[i]+"UnBinned_lj1mass_Xbb60_pT4510_SBRangeChange_QCD_DPG.pdf");
   //gPad->delete();
  }

...
void fitsForFTest_dat()
{
  ...
  //TTree* tree = getTTree() ;
  TString data_1517 = "../data.root";
  //TString data_1517 = "800600-pTfull-nominal_MC16a.root";
  TFile File_Ttree(data_1517);
  //TTreeReader readtree("Ztag", &File);
  TTree *tree = (TTree*)File_Ttree.Get("Ztag");
  float lj1_mass; 
  int lj1_isXbb_60;
  int lj1_nVRjets;
  float lj1_pt;
  tree->SetBranchAddress("lj1_mass", &lj1_mass);
  tree->SetBranchAddress("lj1_isXbb_60", &lj1_isXbb_60); 
  tree->SetBranchAddress("lj1_pt", &lj1_pt);
  tree->SetBranchAddress("lj1_nVRjets", &lj1_nVRjets); 
  TTree* tree_mass = new TTree("tree_mass","tree_mass") ;
  //Double_t* lj1_Xbb60 = new Double_t ;
  float lj1_Xbb60;
  tree_mass->Branch("lj1_Xbb60_mass",&lj1_Xbb60) ;
  int N = tree->GetEntries();
  for (int i = 0; i < N; ++i) {
    tree->GetEntry(i);
    if (lj1_isXbb_60>=1 && (lj1_pt >=450 && lj1_pt < 1000) && lj1_nVRjets >=2)
    { 
      lj1_Xbb60 = lj1_mass;
  ...
  TString data_18 = "../data18.root";
  TFile File_Ttree18(data_18);
  TTree *tree_18 = (TTree*)File_Ttree18.Get("Ztag"); 
  float lj1_mass_18; 
  int lj1_isXbb_60_18;
  int lj1_nVRjets_18;
  float lj1_pt_18;
  tree_18->SetBranchAddress("lj1_mass", &lj1_mass_18);
  tree_18->SetBranchAddress("lj1_isXbb_60", &lj1_isXbb_60_18); 
  tree_18->SetBranchAddress("lj1_pt", &lj1_pt_18);
  tree_18->SetBranchAddress("lj1_nVRjets", &lj1_nVRjets_18);  
  int n = tree_18->GetEntries();
  for (int i = 0; i < n; ++i) {
    tree_18->GetEntry(i);
    if (lj1_isXbb_60_18>=1 && (lj1_pt_18 >=450 && lj1_pt_18 < 1000) && lj1_nVRjets_18 >=2)
    { 
      lj1_Xbb60 = lj1_mass_18;
      //RooDataSet dh_mass("dh_mass","dh_mass",RooArgSet(lj1_mass),Import(*tree)) ;
      tree_mass->Fill();
    }
                              }                 
  ...
  RooRealVar lj1_Xbb60_mass("lj1_Xbb60_mass","lj1_Xbb60_mass",50,150) ;
  ...
  RooDataSet dh_mass("dh_mass","dh_mass",RooArgSet(lj1_Xbb60_mass),Import(*tree_mass)) ;
    
  lj1_Xbb60_mass.setRange("SL", 50, 80);
  ...
  //TString FuncType[1] = { "3rd_Poly"};
   //TString FuncType[6] = { "1st_exp", "2nd_exp", "3rd_exp", "4th_exp", "5th_exp","6th_exp"};
   //TString FuncType[2] = { "2nd_exp"};
   TString FuncType[1] = {"3rd_Poly"};
   //TString FuncType[4] = { "2nd_Poly", "3rd_Poly", "4th_Poly","5th_Poly"};
  //TString FuncType[1] = {"2nd_Poly"};
  //TString FuncType[1] = {"1st_exp"};
  //TString FuncType[3] = {"2nd_Poly", "3rd_Poly", "4th_Poly"};
  //TString FuncType[1] = {"5th_Poly" };
  double npars = 0;
  RooGenericPdf gp;
  double N_yield = 110918;
  for(int i=0; i<1; i++)
  { 
    //TString frame = FuncType[i]
    RooPlot* frame = lj1_Xbb60_mass.frame(Title("Unbinned data shown in default frame binning")) ;
    ...
    {   
        ...
        /*RooRealVar c0("c0","c0",1,-100,1e5);
        RooRealVar c1("c1","c1",1,-100,1e3);
        ...
        gp.plotOn(frame, Range(50,150),NormRange("SL,SU"),LineStyle(kDashed));
        //gp.paramOn(frame);
    }
    ...
    {  
       ...
       RooRealVar c0("c0","c0",10,0,1e3);
       RooRealVar c1("c1","c1",10,-1e3,1e3);
       RooRealVar c2("c2","c2",1,-1e2,1e3);
       RooRealVar c3("c3","c3",-1,-1e2,1e3);
       
       /*RooRealVar c0("c0","c0",10,0,1e2);
       RooRealVar c1("c1","c1",10,-1e2,1e2);
       RooRealVar c2("c2","c2",1,-10,1e2);
       RooRealVar c3("c3","c3",-1,-10,1e2);*/
    ...
    {  
       ...
       /*RooRealVar c0("c0","c0",100,0,1e8);
       RooRealVar c1("c1","c1",-10,-1e3,1e6);
       RooRealVar c2("c2","c2",-10,-1e4,1e6);
       RooRealVar c3("c3","c3",-1,-100,1e6);
       RooRealVar c4("c4","c4",1,-10,100);*/
    ...
    {   
        ...
        RooRealVar c3("c3","c3",70,-200,200);
        RooRealVar c4("c4","c4",-70,-200,200);
        RooRealVar c5("c5","c5",0,-200,200);
        RooGenericPdf gp("gp","GenericPDF","exp(c0*(lj1_Xbb60_mass/100) + c1*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100) + c2*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)+c3*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)+c4*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)+c5*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100))", RooArgSet(c0,c1,c2,c3,c4,c5,lj1_Xbb60_mass)) ;
        gp.fitTo(dh_mass, Range("SL,SU"));
        gp.plotOn(frame, Range("SL,SU"), NormRange("SL,SU"), LineColor(kBlue)) ;
        gp.plotOn(frame, Range(50,150),NormRange("SL,SU"),LineStyle(kDashed));
        //gp.paramOn(frame);
    ...
    {   
        ...
        RooRealVar c6("c6","c6",0.3,-10,10);
        RooGenericPdf gp("gp","GenericPDF","exp(c0*(lj1_Xbb60_mass/100) + c1*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100) + c2*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)+c3*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)+c4*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)+c5*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)+c5*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100))", RooArgSet(c0,c1,c2,c3,c4,c5,c6,lj1_Xbb60_mass)) ;
        gp.fitTo(dh_mass, Range("SL,SU"));
        gp.plotOn(frame, Range("SL,SU"), NormRange("SL,SU"), LineColor(kBlue)) ;
        gp.plotOn(frame, Range(50,150),NormRange("SL,SU"),LineStyle(kDashed));
        //gp.paramOn(frame);
  ...
  //RooPlot* frame3 = lj1_mass.frame(Title("Unbinned data shown in default frame binning")) ;
    ...
    TString text_Chi2=Form("#chi2 = %0.3f",chi2*(70-npars));
  ...
  TLatex * pT = new TLatex(0.15,0.41,"pT=[450,1000] GeV");
  pT->SetNDC();
  pT->SetTextFont(42);
  pT->SetTextColor(1);
  pT->SetTextSize(0.04);
  ...
  probi->Draw();
   ...
   //gPad->SetTicks();
   gPad->SaveAs("./FTestFits/"+FuncType[i]+"UnBinned_lj1mass_Xbb60_pT4510_SBRangeChange_incdata18.pdf");
   //gPad->delete();
  }

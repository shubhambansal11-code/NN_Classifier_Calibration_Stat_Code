...
void fitsForFTest_pT56_dat()
{
  ...
  for (int i = 0; i < N; ++i) {
    tree->GetEntry(i);
    if (lj1_isXbb_60>=1 && (lj1_pt >=500 && lj1_pt < 600) && lj1_nVRjets >=2)
    { 
      lj1_Xbb60 = lj1_mass;
  ...
  for (int i = 0; i < n; ++i) {
    tree_18->GetEntry(i);
    if (lj1_isXbb_60_18>=1 && (lj1_pt_18 >=500 && lj1_pt_18 < 600) && lj1_nVRjets_18 >=2)
    { 
  ...
  TLatex * pT = new TLatex(0.15,0.41,"pT=[500,600] GeV");
  pT->SetNDC();
  pT->SetTextFont(42);
  pT->SetTextColor(1);
  pT->SetTextSize(0.04);
  ...
  probi->Draw();
   ...
   //gPad->SetTicks();
   gPad->SaveAs("./FTestFits/"+FuncType[i]+"pT56_UnBinned_lj1mass_Xbb60_SB5080_dat18.pdf");
   //gPad->delete();
  }

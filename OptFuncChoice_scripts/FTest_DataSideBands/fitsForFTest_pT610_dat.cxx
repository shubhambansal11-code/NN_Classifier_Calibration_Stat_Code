...
void fitsForFTest_pT610_dat()
{
  ...
  for (int i = 0; i < N; ++i) {
    tree->GetEntry(i);
    if (lj1_isXbb_60>=1 && (lj1_pt >= 600 && lj1_pt < 1000) && lj1_nVRjets >=2)
    { 
      lj1_Xbb60 = lj1_mass;
  ...
  for (int i = 0; i < n; ++i) {
    tree_18->GetEntry(i);
    if (lj1_isXbb_60_18>=1 && (lj1_pt_18 >=600 && lj1_pt_18 < 1000) && lj1_nVRjets_18 >=2)
    { 
  ...
  { 
    ...
    {  
       npars = 5; 
       RooRealVar c0("c0","c0",100,-10,1e7);
    ...
    */
    ...
    {   
        ...
        RooGenericPdf gp("gp","GenericPDF","exp(c0*(lj1_Xbb60_mass/100) + c1*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100) + c2*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)+c3*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)+c4*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)+c5*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)+c5*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100)*(lj1_Xbb60_mass/100))", RooArgSet(c0,c1,c2,c3,c4,c5,lj1_Xbb60_mass)) ;
        gp.fitTo(dh_mass, Range("SL,SU"));
        gp.plotOn(frame, Range("SL,SU"), NormRange("SL,SU"), LineColor(kBlue)) ;
        gp.plotOn(frame, Range(50,150),NormRange("SL,SU"),LineStyle(kDashed));
        //gp.paramOn(frame);
  ...
  //RooPlot* frame3 = lj1_mass.frame(Title("Unbinned data shown in default frame binning")) ;
   ...
   TLatex *tex1 = new TLatex();
    ...
    {
      tex1= new TLatex(lx,ly,"dijet FF: exp(#Sigma_{i=1 to 7} a_{i}*(x/100)^{i})");
    }
    tex1->SetNDC();
  ...
  probi->Draw();
   ...
   //gPad->SetTicks();
   gPad->SaveAs("./FTestFits/"+FuncType[i]+"pT610_UnBinned_lj1mass_Xbb60_SB5080_dat18.pdf");
   //gPad->delete();
  }
